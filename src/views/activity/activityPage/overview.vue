<template>
  <div :loading="edit_confirm_loading">
    <el-card class="intro-card-component">
      ...
      <!-- 活动配图展示 -->
      <div class="images">
        <el-image style="margin-right: 1em; height: 20em" v-for="(img, img_i) in image_urlList" :key="img_i" :src="img">
      ...
      <div class="projmembers_box">
        ...
        <div class="pmb_members">
          <!-- 点击邀请好友 -->
          <i v-if="UserIdentity == 'creator'" class="el-icon-circle-plus-outline" style="margin-top: 1em; font-size: 1.8em" @click="ClickShowInviteFriend"></i>
          <div class="pmb_member_item" v-for="(member, index) in participant_list" :key="index">
            <el-popover placement="bottom" :title="member.name" width="200" trigger="click">
      ...
      <el-dialog class="icc_adminMember_dialog" :v-if="isShow_dialog_adminAllMembers" :visible.sync="isShow_dialog_adminAllMembers">
        <span class="icc_adminMember_dashBoard">
          ...
          <div class="icca_memberline" v-for="(mem, index) in participant_list" :key="index">
            <el-avatar @click.native="CheckUserDetail(mem)" class="icca_memerline_avatar" :src="creator_data.avatar">
            </el-avatar>
      ...
      <span v-if="state_val == 1">
        <h2>已结束报名！活动正在进行中</h2>
        <el-button v-if="UserIdentity == 'creator'" type="text" @click="ClickCreatorFinishAct">结束活动</el-button>
      </span>
      ...
      <!-- 某个用户的详情查看 -->
      <el-dialog class="mac_user_detail_dialog" :v-if="isShow_dialog_user" :visible.sync="isShow_dialog_user" title="其他用户信息">
        <div>
        ...
        <ReportBox selection="test" targetType="1" :userId="other_user_detail_data.id" :name="other_user_detail_data.name" />
      </el-dialog>
    </el-card>

    <!-- 评价的卡片 -->
    <ActivityMarkCard style="margin-left: 1em" v-if="rate_val != -1 && state_val == 2" :activity_id="activity_id" :user_id="user_id" :hadrate="rated" :showVal="rate_val"
      @MarkActivity="ComponentsMarkActivity"></ActivityMarkCard>
    <!-- 活动投票卡片 -->
    <el-col :span="12" v-if="is_member">
        ...
        <div v-for="(poll, index) in Polls" :key="index">
          <div v-if="poll.notVoted*(Date.parse(poll.deadline)-nowTime)">
            ...
            <span v-if="poll.notCommitted">
              ...
              <el-button style="margin-left:50px ;" v-if="is_creator" @click="pollToDelete=poll.poll_id;deleteVote()">删除投票</el-button>
            </span>
          </div>
        </div>
      </el-card>
      <el-card>
        ...
        <div v-for="(poll, index) in Polls" :key="index">
          <div v-if="!poll.notVoted" style="margin-bottom: 30px">
            ...
            <div v-for="(option, optindex) in poll.option[0].answers" :key="optindex">
              <div style="
                ...
                <span v-text="
                  ...
                  " style="
                    ...
                    height: 30px;
                  "></span>
                <span style="line-height: 30px" v-if="option.value == poll.selected" v-html="option.text + ' ✔'"></span>
                <span style="line-height: 30px" v-else v-html="option.text"></span>
              </div>
            </div>
            <el-button style="margin-left:50px ;" v-if="is_creator" @click="pollToDelete=poll.poll_id;deleteVote()">删除投票</el-button>
          </div>
      ...
      <el-card>
        <span>已结束投票</span>
        <div v-for="(poll, index) in Polls" :key="index">
          <div v-if="Date.parse(poll.deadline)-nowTime<=0" style="margin-bottom: 30px">
            <h3>{{ poll.option[0].question }}</h3>
            <div v-for="(option, optindex) in poll.option[0].answers" :key="optindex">
              <div style="
                  background-color: #e1e8ed;
    ...
    <el-dialog v-bind="$attrs" v-on="$listeners" title="修改活动信息" :before-close="handleClose_edit" :visible.sync="isShow_dialog_edit">
        ...
        <el-form ref="create_activity" :model="newact_form" :rules="newact_rules" size="medium" label-width="100px" label-position="top">
          <el-col :span="24">
            <el-form-item label="活动标题" prop="title_name">
              <el-input v-model="newact_form.title_name" placeholder="请输入活动标题" clearable :style="{ width: '100%' }">
              </el-input>
            </el-form-item>
          </el-col>
          <el-col :span="24">
          ...
          <el-col :span="24">
            <el-form-item label="活动地点" prop="address_name">
              <el-input v-model="newact_form.address_name" placeholder="请输入活动地点" clearable prefix-icon="el-icon-location-information" :style="{ width: '100%' }"></el-input>
            </el-form-item>

            <el-form-item label="详细地址" prop="address_formatted">
              <el-input v-model="newact_form.address_formatted" placeholder="可以根据地图选择详细地址" clearable prefix-icon="el-icon-location-information" :style="{ width: '100%' }"></el-input>
            </el-form-item>

            <el-form-item>
              <el-button type="primary" @click="innerVisible = !innerVisible">选择地图地址</el-button>
            </el-form-item>
          </el-col>
          <el-col :span="12">
          ...
          <el-col :span="12">
            <el-form-item label-width="-3px" label="参与人数上限" prop="capacity">
              <el-input-number :disabled="newact_form.limit_capacity != '1'" v-model="newact_form.capacity" :step="1" :min="2"></el-input-number>
            </el-form-item>
          </el-col>

          <el-col :span="24">
            <el-form-item label="标签" required>
              <el-checkbox-group v-model="newact_form.tags" @change="TagGroupChange">
                <span v-for="(tag, tag_index) in optional_tags" :key="tag_index">
      ...
      <span v-if="primitiveData != -1">
        <MapChoose :dialogShow.sync="innerVisible" @locationSure="locationSure" :primitiveData_comp="primitiveData"></MapChoose>
      </span>
    </el-dialog>

    <!-- 邀请好友的dialog -->
    <el-dialog :v-if="showDialog_inviteFriend" :visible.sync="showDialog_inviteFriend" title="邀请好友参与活动">
      <span>
        <el-tree :data="friend_tree_data" show-checkbox default-expand-all node-key="id" ref="tree" highlight-current props="{
          children: 'children',
          label: 'label'
        }">
        </el-tree>
        <el-button @click="resetChecked">清空选中</el-button>
        <el-button type="primary" @click="InviteFriends">确认邀请好友</el-button>
      </span>
    </el-dialog>
  </div>
</template>
