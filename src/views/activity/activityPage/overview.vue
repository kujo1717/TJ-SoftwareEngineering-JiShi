<template>
  <div :loading="edit_confirm_loading">
    <el-card class="intro-card-component">
      ...
      <el-dialog class="icc_adminMember_dialog" :v-if="isShow_dialog_adminAllMembers" :visible.sync="isShow_dialog_adminAllMembers">
        <span class="icc_adminMember_dashBoard">
          <div style="margin-top: -1em;font-size: 2em;color: black;font-weight: bold;">
            参与人员
          </div>
    ...
    <el-col :span="12" v-if="is_member">
      ...
      <!--span v-if="hasVote"-->
      <!-- 展示投票 -->
      <el-button @click="getPoll();calculatePercentage()">查看本活动投票</el-button>
      <el-card>
        <span>待投票</span>
        <div v-for="(poll, index) in Polls" :key="index">
          <div v-if="poll.notVoted">
        ...
        <div v-for="(poll, index) in Polls" :key="index">
          <div v-if="!poll.notVoted" style="margin-bottom:30px">
            ...
            <div v-for="(option, optindex) in poll.option[0].answers" :key="optindex">
              <div style="background-color: #e1e8ed;margin-bottom:10px;height:30px" :style="{ width: Math.round(option.votes/poll.totalVotes*100) + '%' }">
                <span v-text="Math.round(option.votes/poll.totalVotes*100) + '%'" style="padding-left:10px;font-weight: bold;padding-right:50px;height:30px"></span>
...
import { getPoll, getVote, postPoll, postVote } from '@/api/poll'
