  ...
  export default {
    ...
    methods: {
      ...
      handleCommand(command){
        ...
        let re=true
        this.friends.forEach(element => {
          console.log(element.id,command.item.id,element.groupid,command.group.groupid)
          if(element.id===command.item.id && element.groupid===command.group.groupid){
            console.log("相等")
            re=false
          }
        });
        console.log(command)
        if(re){
          this.friends[this.friends.indexOf(command.item)].groupid=command.group.groupid
        }
        else{
      ...
      handleCopyCommand(command){
        ...
        console.log(command)
        if(re){
          this.friends.push({"avatar":command.item.avatar,"email":command.item.email,"groupid":command.group.groupid,"id":command.item.id,
          "introduce":command.item.introduce,"name":command.item.name,"rootname":command.item.rootname})
        }
        else{
          this.$message.error('目标组里已经存在当前好友');
        }
